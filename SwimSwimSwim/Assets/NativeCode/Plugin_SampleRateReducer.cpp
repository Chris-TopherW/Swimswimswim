//
//  Plugin_SampleRateReducer.cpp
//  AudioPluginDemo
//
//  Created by Chris Wratt on 22/08/16.
//
//

#include "AudioPluginUtil.h"

namespace SampleRateReducer
{
    enum Param
    {
        P_REDUCE,
    ...
    int InternalRegisterEffectDefinition(UnityAudioEffectDefinition& definition)
    {
        ...
        RegisterParameter(definition, "Decimation", "", 1.0, 100.0f, 1.0f, 1.f, 60.f, P_REDUCE, "");
        return numparams;
    }
    ...
    UNITY_AUDIODSP_RESULT UNITY_AUDIODSP_CALLBACK CreateCallback(UnityAudioEffectState* state)
    {
        ...
        effectdata->data.currentModulo = 0;
        effectdata->data.reduceAsInt = 0;
    ...
    UNITY_AUDIODSP_RESULT UNITY_AUDIODSP_CALLBACK ProcessCallback(UnityAudioEffectState* state, float* inbuffer, float* outbuffer, unsigned int length, int inchannels, int outchannels)
    {
        ...
        for (unsigned int n = 0; n < length; n++)
        {
            ...
            {
                if(data->currentModulo == 0)
                    outbuffer[n * outchannels + i] = inbuffer[n * outchannels + i];
                else if(n > 1)
                    outbuffer[n * outchannels + i] = outbuffer[n * outchannels + i - 1];
                else
                    ;

                //mix control
                outbuffer[n * outchannels + i] = (outbuffer[n * outchannels + i] * (1.0f - data->p[P_MIX])) + (data->p[P_MIX] * inbuffer[n * outchannels + i]);
            }
            data->currentModulo++;
            data->reduceAsInt = (int)data->p[P_REDUCE];
            data->currentModulo = data->currentModulo % data->reduceAsInt;
        }
