...
namespace BitCrasher
{
    ...
    int InternalRegisterEffectDefinition(UnityAudioEffectDefinition& definition)
    {
        ...
        RegisterParameter(definition, "Bit crush amount", "", 120.0f, 123.0f, 120.0f, 0.01f, 1.0f, P_CRUSH, "");
        return numparams;
    }
    ...
    UNITY_AUDIODSP_RESULT UNITY_AUDIODSP_CALLBACK ProcessCallback(UnityAudioEffectState* state, float* inbuffer, float* outbuffer, unsigned int length, int inchannels, int outchannels)
    {
        ...
        for (unsigned int n = 0; n < length; n++)
        {
            for (int i = 0; i < outchannels; i++)
            {
                ...
                outbuffer[n * outchannels + i] = inbuffer[n * outchannels + i] / pow(2.0f,data->p[P_CRUSH]);
                outbuffer[n * outchannels + i] = outbuffer[n * outchannels + i] * pow(2.0f,data->p[P_CRUSH]);
                //mix level
